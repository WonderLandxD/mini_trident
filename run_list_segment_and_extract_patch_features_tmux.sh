...
for i in "${!parts[@]}"; do
  ...
  cmd="cd \"$root_dir\"; python list_segment_slide_and_extract_patch_features.py --list_json \"$part\" --job_dir \"$job_dir\" --gpu $gpu --mag $mag --save_patches_type \"$save_patches_type\" --encoder \"$encoder\" --precision \"$precision\" --feat_num_workers $feat_num_workers --verbose"
  tmux new-session -d -s "$session"
  tmux send-keys -t "$session" "$cmd" C-m
  echo "tmux: $session -> GPU $gpu, encoder: $encoder, save_patches_type: $save_patches_type, mag: $mag"
done
